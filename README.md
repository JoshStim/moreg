...
```
  ...
  * `./output/{TASK}_{PED}_{METHOD}/motion_regressor_files.txt`: A .txt file containing paths to all motion regressor files within a specific scan and motion correction method.

  * `./output/{TASK}_{PED}_{METHOD}/{SUBID}/EVs`: The EVs folder for a given scan and motion correction method. Includes task events as well as motion regressors (depends on the method used)

  * `./output/{TASK}_{PED}_{METHOD}/{SUBID}/tfMRI_{TASK}_{PED}_hp200_s4.feat`: Contains outputs from the level 1 FEAT analysis.
  ...
  * `./output/{TASK}_{PED}_{METHOD}/{SUBID}/tfMRI_{TASK}_{PED}.nii.gz`: Raw data for a given participant's scan.
